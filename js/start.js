    ...
    setTimeout(function(){
        ...
        start_Enter[0].onmouseover = function (){
            if(sb==1){
                ...
                reply[0].children[1].innerHTML = "后续正在开发中qwq";
                console.log("进去了");
            }else{
                alert("后续正在开发中qwq");
            }
        ...
        start_Enter[0].onclick = function(){
            console.log("动起来");
            start_Enter[0].children[0].style.background = "url(./image/xsh.gif)";
            start_Enter[0].children[0].style.backgroundSize = "100% 100%";
        }
    },1100)     //定时器(在1.1秒后执行)
    if(height>width){
        start_Enter[0].children[0].style.top = "-13vw";
        start_Enter[0].children[0].style.width = "8vh";
        start_Enter[0].children[0].style.height = "8vh";
